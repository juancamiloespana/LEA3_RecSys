...
def recomendar(user_id=list(usuarios['user_id'].value_counts().index)):
    ...
    ratings=pd.read_sql('select *from ratings_final where user_id=3371 or user_id=6772',conn)
...
gridsearchKNNWithMeans = GridSearchCV(KNNWithZScore, param_grid, measures=['mae'], \
                                      cv=2, n_jobs=-1)
...
gs_model=gridsearchKNNWithMeans.best_estimator['mae'] ### mejor estimador de gridsearch
...
#####
model.predict(uid=52853, iid='0345438329',r_ui='') ### uid debía estar en número e isb en comillas
